...
> ⚠️ **동명 토큰이 다른 체인에 있습니다.** 이더리움과 Solana에 `YUMI`가 있고,
> Base에도 비슷한 이름(SUMI·LUMI·YUMY)이 여럿 있습니다. ERC-20 심볼은 유일하지 않아서
> **누구나 같은 이름으로 토큰을 만들 수 있습니다.**
>
> **반드시 주소로 확인하세요.** 주소는 [`docs/WALLETS.md`](docs/WALLETS.md)에 있습니다. (D-027)
...
- [`docs/LOGO.md`](docs/LOGO.md) — 로고를 지갑·거래 화면에 띄우는 방법 (체인에 올라가지 않습니다)
...
**기록의 원본은 이 저장소입니다.** 채널은 새 기록이 올라온 것을 알리는 곳이고,
[`log/`](log/)에 있는 것이 원본입니다. 둘이 다르면 저장소가 맞습니다.

🔴 **주소는 [`docs/WALLETS.md`](docs/WALLETS.md) 한 곳에만 올립니다.**
DM·검색 결과·다른 사이트에서 받은 주소는 믿지 마세요. 심볼은 신원이 아니고 주소가 신원입니다.
...
| | |
|---|---|
| 코드 (`src/` · `test/` · `script/` · `scripts/`) | **MIT** — 자유롭게 쓰세요 |
| 문서 (`*.md`) | 인용·번역 자유. **출처를 밝혀주세요** |
| 로고 (`assets/`) | **이 프로젝트를 가리킬 때만.** 다른 토큰·계정의 표식으로 쓸 수 없습니다 |

코드를 MIT로 두는 이유는 **감출 것이 없기 때문**입니다. OpenZeppelin 표준을 상속했을 뿐이고,
누가 그대로 복제해도 우리가 잃는 것이 없습니다.

**로고만 조건을 답니다.** 같은 이름과 로고를 쓴 토큰이 나타났을 때
「심볼은 신원이 아니다」 외에 말할 것이 하나는 있어야 합니다. (D-035)
